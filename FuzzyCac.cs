...
namespace TobaccoExe
{
    class FuzzyCac
    {
		...
		const int DMIN = -100;
		const int FMAX = 100;
		int[] PFF = new int[4] { 0, 12, 24, 48 };
		/* 输入量 D语言值特征点 */
		int[] DFF = new int[4] { 0, 16, 32, 64 };
		/* 输出量 U语言值特征点 */
		int[] UFF = new int[7] { 0, 15, 30, 45, 60, 75, 90 };
		/* 采用了调整因子的规则表 , 大误差时偏重误差 , 小误差时偏重误差变化 */
		/*a0=0.3,a1=0.55,a2=0.74,a3=0.89 */
		int[,] rule = new int[7, 7] {
		...
		public float fuzzyRes(float P, float D) /* 模糊运算引擎 */
		{
			...
			DF[1] = FMAX - DF[0];
			/* 使用误差范围优化后的规则表 rule[7][7]*/
			/* 输出值使用 13 个隶属函数 , 中心值由 UFF[7]指定*/
			/* 一般都是四个规则有效 */
